...
const users = [
  ...
  const olderThan30 = users.reduce((counter,user) =>  user.age > 30 ? counter+1: counter,0) /// якщо знайшло додай наш каунтер якщо ні залиш
  console.log(olderThan30) /// 2 
  
  //•	Count how many users live in Chicago     			-> 2
  const liveInChicago = users.reduce((counter, user) => user.address.city.toLowerCase() === 'chicago' ? counter + 1 : counter,0)
  console.log(liveInChicago)
  
  //•	Count how many users live in IL     -> 3
  const liveInIllinois = users.reduce((counter, user) => user.address.state.toLowerCase() === 'iL' ? counter + 1 : counter, 0)
  ...
  */
  ...
  // Find the most expensive product   -> MacBook Pro 16-inch
  const mostExpensive = appleStore.reduce((max, curr) => max.price > curr.price ? max: curr, {}); /// insted of 0 we have {}
  console.log(mostExpensive.productName); ///// we return whole name
  // Find the least expensive product   -> AirPods Pro
  ...
  // Find all the product names   -> [ 'iPhone 14 Pro', 'MacBook Pro 16-inch', 'iPad Air', 'Apple Watch Series 7', 'AirPods Pro' ]
  const allProductNames = appleStore.map((product) => product.productName);   // creat  new arr 
  ...
  */
  ...
  }, {}); //// we added our value empty object, if the price works we will add to our onj
  ...
  const classicBooks = bookstore.reduce((acc, curr) => {
    if(curr.genre.toLowerCase() === 'classic') acc.push(curr.title);
    return acc;
  }, []) /// start empty arr
