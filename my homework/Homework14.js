...
const isPerfectSquare = (number) => number >= 0 && Math.sqrt(number)  % 1 === 0
                                           ///Math.sqrt(number) % 1 === 0 — перевіряємо, що корінь з числа не має дробової частини (тобто ціле число).
...
const capsOdds = (arr) =>  arr.map((element, index) => index % 2 !== 0 ? element.toUpperCase() : element)
